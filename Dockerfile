...
# pre-install the CPU wheel and requirements then resolve to it. Build with
# `--build-arg TORCH_FLAVOR=gpu` (and run with `--gpus all`) to keep the CUDA build instead.
