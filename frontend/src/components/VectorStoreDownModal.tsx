...
/** Shown when an opt-in compose-profile vector store is selected but not reachable.
 * Offers the exact start command plus a re-check, rather than the app trying to start a
 * sibling container itself. */
export function VectorStoreDownModal({
  open,
  onClose,
  onReachable,
  onContinueAnyway,
  ...
  return (
    <Modal open={open} onClose={onClose} title={`${storeLabel} isn't running`} icon={<AlertTriangleIcon size={16} className="text-warn" />}>
      <p className="text-xs text-ink-dim mb-3 leading-relaxed">
        {storeLabel} sits behind an opt-in compose profile, so it doesn't start with a plain{' '}
      ...
      <div className="flex items-center gap-2 rounded-control border border-line bg-surface-2/70 px-3 py-2 mb-4">
        ...
        <code className="text-xs text-ink flex-1 overflow-x-auto whitespace-nowrap">{startCommand}</code>
        <button
          type="button"
          onClick={copy}
          title="Copy command"
