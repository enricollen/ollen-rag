"""Tests for ChromaBackend: a real in-process PersistentClient on a tmp dir (no HTTP mocking needed),
covering capability, node round-trip, meta reconstruction, admin/browse, bucket aggregation and dedup."""
import chromadb
import pytest
from llama_index.core.schema import TextNode
from src.exceptions import VectorStoreError
from src.factories.vector_store import QueryMode, VectorStoreFactory
from src.providers.vector_stores.chroma import ChromaBackend
from src.settings import Settings

SETTINGS = Settings(_env_file=None)
IDX = "ollen_rag_chroma"

def _backend(tmp_path) -> ChromaBackend:
    """ChromaBackend wired to a throwaway on-disk client under the test's tmp dir."""
    return ChromaBackend(SETTINGS, client=chromadb.PersistentClient(path=str(tmp_path / "chroma")))
...
def test_registered_under_chroma():
    assert VectorStoreFactory._registry.get("chroma") is ChromaBackend

def test_supported_modes_dense_only(tmp_path):
    assert _backend(tmp_path).supported_query_modes == {QueryMode.DENSE}

# --- lifecycle / dim ---

def test_ensure_ready_stamps_dim(tmp_path):
    b = _backend(tmp_path)
    b.ensure_ready(IDX, 768)
    assert b.get_index_dim(IDX) == 768

def test_get_index_dim_missing_is_none(tmp_path):
    assert _backend(tmp_path).get_index_dim("ollen_rag_nope") is None

# --- ingest / retrieve ---

def test_add_and_retrieve_dense(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [
...
def test_retrieve_bucket_filter(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [
...
def test_add_empty_is_noop(tmp_path):
    _backend(tmp_path).add_nodes(IDX, [])  # must not raise

def test_retrieve_missing_index_raises(tmp_path):
    with pytest.raises(VectorStoreError):
        _backend(tmp_path).retrieve("ollen_rag_nope", "q", [0.1, 0.2, 0.3], QueryMode.DENSE, 1, None, "and")

# --- meta ---

def test_meta_roundtrip_reconstructs_nested_shape(tmp_path):
    b = _backend(tmp_path)
...
def test_meta_missing_is_none(tmp_path):
    b = _backend(tmp_path)
    b.ensure_ready(IDX, 3)  # created but no meta recorded yet
    assert b.get_index_meta(IDX) is None

def test_set_meta_preserves_dim(tmp_path):
    b = _backend(tmp_path)
    b.ensure_ready(IDX, 3)
...
def test_get_documents_scoped_to_bucket(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [
...
def test_buckets_and_bucket_files(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [
        _node("1", "a.pdf", "b1", file_hash="h1"),
        _node("2", "a.pdf", "b1", file_hash="h1"),  # same file, another chunk
...
def test_find_duplicate_different_bucket_is_none(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [_node("x", "a.pdf", "b1", file_hash="hash1")])
    assert b.find_duplicate_file(IDX, "hash1", "b2") is None

def test_find_duplicate_bucketless_scope(tmp_path):
    b = _backend(tmp_path)
...
def test_find_duplicate_missing_index_is_none(tmp_path):
    assert _backend(tmp_path).find_duplicate_file("ollen_rag_nope", "h", "b1") is None

# --- delete ---

def test_delete_index_removes_collection(tmp_path):
    b = _backend(tmp_path)
    b.add_nodes(IDX, [_node("x", "a.pdf", "b1")])
    b.delete_index(IDX)
